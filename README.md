...
- 演出发布支持多场次、多票档和富文本详情页，详情内容、退票时间节点、场次和票档都会写入数据库，重新打开后台编辑页可以继续维护。
- 演出售卖状态由开售时间和实时库存自动判断：即将开售、热卖中、已售罄或已结束，前台按钮随状态变化。
- 非电影演出按场次、票档、观演人实名信息购票，单次最多 4 张；电影票不要求观演人，按电影院、影厅、放映时间和座位购票，单次最多 4 个座位。
- 后台有独立的首页推荐功能，可按热门推荐和分类板块选择最多 4 个内容；演出发布和电影管理页不再承担首页挂载设置。
- 后台有独立的电影院管理功能；普通场馆管理只维护剧院和体育场馆，电影院按影厅和座位图维护。
- 系统启动时会补齐基础场馆数据：已有城市每个生成 5 个电影院、5 个体育场馆和 5 个剧院；每个电影院默认 2 个影厅和 4 排 6 座。
...
- 首页聚合轮播、分类、热门推荐、各分类推荐、即将开售、正在售票、热门城市、热门场馆和电影。
- 搜索页支持关键词、城市、分类、售卖状态筛选。
- 演出详情展示海报、富文本详情、场次、票档、服务标签、购票/退票/入场说明。
...
- 演出档案：新增、编辑、发布、下架、删除、富文本详情排版和退票时间节点。
- 图片资源：支持上传文件，也支持导入本机图片到 `uploads/admin/`，数据库只保存 `/uploads/...` 相对路径。
- 场馆与座位：普通场馆维护剧院和体育场馆；电影院在独立模块维护影厅和座位图。
...
- 未开售时可提交预约提醒；正在售卖时可进入抢票或选座。
- 抢票接口会根据批次、票档、用户、库存和重复成功记录生成请求结果。
- 选座流程包含锁座、释放锁座、创建订单、模拟支付、出票；电影票不要求观演人，演出票每张票对应一位观演人。
...
后端测试启动了 Spring Boot 上下文并通过；前端构建成功，同时 Vite 输出了大 chunk 警告和第三方依赖注释提示，这些属于构建优化提示，不影响当前课程演示包产物生成。完整浏览器联调和 `scripts/rush_test.py` 并发抢票脚本仍建议在 MySQL、Redis、后端和前端都启动后补充截图验证。
